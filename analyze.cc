...
std::string funcTypeToString(bucket *func){
	...
	if(funcType != NULL){
		for(int i=0; i<funcType->parameterstype.size(); i++){
			if(funcType->parameterstype[i] == DATATYPE_INT || funcType->parameterstype[i] == DATATYPE_BOOL){
				res +=  typeEncoding(funcType->parameterstype[i]);
			}
			else if(funcType->parameterstype[i] == DATATYPE_FUNC){
				
			}else if(funcType->parameterstype[i] == DATATYPE_UNKOWN){
		...
		res += "=>";
		if(funcType->returnvaltype == DATATYPE_INT || funcType->returnvaltype == DATATYPE_BOOL){
			res +=  typeEncoding(funcType->returnvaltype);
		}
		else if(funcType->returnvaltype == DATATYPE_FUNC){
			
		}else if(funcType->returnvaltype == DATATYPE_UNKOWN){
...
int checkParameterIfMatch(std::vector<int> curparameters, std::vector<int> preparameters){
	if(curparameters.size() != preparameters.size()){
		return -1;
	}
	for(int i=0; i< curparameters.size(); i++){
		if(curparameters[i] != DATATYPE_UNKOWN || curparameters[i] != preparameters[i]){
			return -1;
		}
	}
	return 1;
}
void setFunctionVariableType(char* varName, std::string functionname, int dataType, std::vector<int> parameters, int returntype){
	std::string varKey(varName);
	if(variableMap[functionname][varKey] != NULL){
		if(variableMap[functionname][varKey]->infer_type == NULL){
			//set infer_type
			variableMap[functionname][varKey]->infer_type = (functionType *)malloc(sizeof(functionType));
			variableMap[functionname][varKey]->infer_type->parameterstype = parameters;
			variableMap[functionname][varKey]->infer_type->returnvaltype = returntype;
		}else{
	...
	|| icode->op == OPNOTEQ){
		...
		if(icode->type == DATATYPE_FUNC){
			...
			while(parameterPointer != NULL){
				...
				parameters.push_back(parameterPointer->type);
		...
		variableMap[functionname][variableName] = (bucket *)malloc(sizeof(bucket));
...
void analyze_func(TreeNode* icode){
	...
	//typeInference(icode->child[0], functionname);
}
